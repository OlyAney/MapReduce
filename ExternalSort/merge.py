                    ...
                    if not chunk_2:
                        ...
                        while True:
                            try:
                                ...
                                break
                    break
            # ну и-за того что flush такой в коде ровно один, всё плохо
            # смотрите - chunk_12 у вас в итоге будет равен суммарному размеру файлов которые вы мержите
            # который может быть ооочень большим
            # правильно было следить за размером chunk_12 на этапе добавления в него новой записи и когда его размер превосходит ваш лимит
            # сбрасывать содерижимое на диск
            # заодно стоило написать вокруг такого выходного и таких выходных итераторов обёртку, чтобы вся логика программы не содержалась
            # в наборе ifов в которых стороннему наблюдателю не так просто разобраться
